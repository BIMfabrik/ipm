(() => {
  ...
  function routeInfo() {
    ...
    return { inKnowledge, inArticle, inTables, root };
  }
  function installSidebar() {
    if (document.querySelector('.dashboard-sidebar')) return;
    const { inKnowledge, inTables, root } = routeInfo();
    const active = inKnowledge ? 'knowledge' : (inTables ? 'tables' : 'home');
    const links = [['home',root,'Overview'],['model',`${root}#graph`,'Model'],['method',`${root}#approach`,'Method'],['knowledge',`${root}knowledge/`,'Knowledge'],['tables',`${root}tables/`,'Tables'],['about',`${root}#about`,'About']];
  ...
  function installNavigation(){const {inKnowledge,inTables}=routeInfo();if(!inKnowledge&&!inTables)return;const nav=document.querySelector('.navlinks');if(nav){nav.removeAttribute('aria-label');nav.innerHTML='';}}
