(() => {
  ...
  function createGlowTexture(color) {
    ...
    gradient.addColorStop(.42,color.replace(')',',.34)').replace('rgb','rgba'));
  ...
  function galaxyNodeObject(node) {
    ...
    group.add(core);
    const sprite = new THREE.Sprite(new THREE.SpriteMaterial({map:createGlowTexture(node.color),transparent:true,depthWrite:false,blending:THREE.AdditiveBlending}));
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    ensureGalaxyOption();
    if (location.hash === '#graph') requestAnimationFrame(() => window.setTimeout(() => scrollToGraph({updateHash:false,smooth:false}),120));
